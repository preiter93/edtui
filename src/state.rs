...
impl EditorState {
    ...
    // pub(crate) fn insert_line(&mut self, index: usize, line: &str) {
    //     let chars: Vec<char> = line.chars().collect();
    ...
    // }
    //
    // pub(crate) fn get(&self, index: Index2) -> Option<&char> {
    //     self.lines.get(index)
    // }
    //
    // pub(crate) fn skip_whitespace(&mut self) {
    //     let index = self.cursor.as_index();
    //     if self.lines.get(index).is_none() {
    //         return;
    //     }
    //     if let Some((_, index)) = self
    //         .lines
    //         .next_predicate(index, |ch| !ch.map_or(true, |x| x.is_whitespace()))
    //     {
    //         self.cursor = index.into();
    //     }
    // }

    // /// Get the next value and index skipping whitespaces. Stops at the
    // /// end of a line and returns the last position.
    // pub fn skip_whitespace(&mut self) {
    //     let start = self.cursor.as_index();
    //     let mut index = self.cursor.as_index();
    //     if let Some(value) = self.lines.get(index) {
    //         if !value.is_whitespace() {
    //             return;
    //         }
    //     }
    //     for (val, ind) in self.lines.iter().from(start) {
    //         if ind.row != start.row {
    //             break;
    //         }
    //         index = ind;
    //         if let Some(v) = val {
